...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 15' do
      let(:magical) { described_class.new(8) }
      it 'returns 7' do
        previous_step = 15
        result = magical.subtract_random_number(15)
  ...
  describe '#play' do
    context 'when the random number is 245' do
      let(:magical) { described_class.new(245) }
    ...
    context 'when the random number is 15' do
      let(:magical) { described_class.new(15) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = magical.play
        expect(result).to be(7)
    ...
    context 'when the random number is -20' do
      let(:magical) { described_class.new(-20) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = magical.play
        expect(result).to be(7)
